...
class Parser:
    ...
    def parse_text(self):
        with open('./config/employees.json') as json_file:
            self.employee_information = json.load(json_file)
    ...
    def __update_count(self, information_array):
        ...
        employee_name = self.employee_information[employee_number]
        if employee_name not in self.count_information.keys():
            self.count_information[employee_name] = {
                'Total': 0,
                'Immed': 0,
                'Later': 0,
                'Emails': 0
        ...
        # update total
        self.count_information[employee_name]['Total'] += 1

        # update immed/later. if not immed, it must be later.
        if employee_name == information_array[7] and information_array[3] == False:
            self.count_information[employee_name]['Immed'] += 1
        elif employee_name != information_array[7] and information_array[3] == False:
            self.count_information[employee_name]['Later'] += 1

        # update emails
        if information_array[3] == True:
            self.count_information[employee_name]['Emails'] += 1
