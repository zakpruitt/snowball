...
class ExcelWriter:
    ...
    def format_headers_sf(self, sheet_name, count_df, buffer, format):
        worksheet = self.writer.sheets[sheet_name]
        for col_num, value in enumerate(count_df.columns.values):
            worksheet.write(0, col_num + buffer, value, self.formats[format])

    def format_headers_af(self, sheet_name, count_df, buffer, format1, format2):
        worksheet = self.writer.sheets[sheet_name]
        for col_num, value in enumerate(count_df.columns.values):
            if col_num % 2 == 0:
    ...
    def format_columns_af(self, sheet_name, dataframe, spacing, format1, format2):
        worksheet = self.writer.sheets[sheet_name]
        for col in range(0, len(dataframe.columns)):
            if col % 2 == 0:
                worksheet.set_column(col, col, spacing, self.formats[format1])
            else:
                worksheet.set_column(col, col, spacing, self.formats[format2])
